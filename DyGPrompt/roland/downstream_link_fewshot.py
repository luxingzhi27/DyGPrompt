...
# Set up logger
logger = setup_logger(f'roland_downstream_link_fewshot_{args.data}')
logger.info(args)
...
for task in task_pbar:
    ...
    task_pbar.set_postfix({'test_auc': f'{np.mean(tmp_auc):.4f}', 'nn_test_auc': f'{np.mean(tmp_nn_auc):.4f}'})
