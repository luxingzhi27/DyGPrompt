...
def log_epoch_stats(logger, epoch, epoch_time, loss, val_auc, val_ap, nn_val_auc=None, nn_val_ap=None):
    ...
    """
    msg = f"Epoch: {epoch} | Time: {epoch_time:.2f}s | Loss: {loss:.4f} | Val AUC: {val_auc:.4f} | Val AP: {val_ap:.4f}"
    if nn_val_auc is not None and nn_val_ap is not None:
        msg += f" | New Node Val AUC: {nn_val_auc:.4f} | New Node Val AP: {nn_val_ap:.4f}"
    logger.info(msg)
